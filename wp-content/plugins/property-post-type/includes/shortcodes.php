...
function property_func( $atts ) {
	...
	$args = array(
		'post_type' => 'property',
		'post_status' => array('publish','reserved','sold'),
		'orderby' => 'menu_order',
		'order' => 'ASC',
		'posts_per_page' => '-1',
		//'paged' => $paged
	...
	while( $query->have_posts()) : $query->the_post();
		...
		switch ($post->post_status) {
			...
			case "reserved":
				$availability = $asking_price = '<font style="color:#E2804F">Reserved</font>';
				break;
			
			case "sold":
				$availability = $asking_price = '<font style="color:#E2804F">Sold</font>';
				break;
		}
		...
		$row = array(
			...
			'TYPE'=>$tag,
			'NAME'=>$category,
			'APPROX GROSS<BR class="vc_visable-xs vc_hidden-sm"/><span class="vc_hidden-xs vc_visable-sm vc_hidden-md vc_hidden-lg">&nbsp;</span>INTERNAL SQ FT'=>$floor,
...
?>
	...
	<div id="field-filters" class="vc_row wpb_row vc_inner vc_row-fluid field-names row-light-green">
			<div class="wpb_column vc_column_container vc_col-md-2 vc_col-sm-12 vc_col-xs-12">
				<div class="vc_column-inner ">
					<div class="wpb_wrapper">
						<div class="wpb_text_column wpb_content_element ">
...
<?php	
	...
	foreach(array_keys($row) AS $f): ?>
		<div class="wpb_column vc_column_container vc_col-md-1 vc_col-sm-1 vc_hidden-sm vc_hidden-xs field-titles<?php echo " C_". str_replace($not_allowed, "_", strip_tags(strtolower($f)) ); if( $row_count == $rows ): echo " last-col"; endif; if( $row_count == 0 ) echo " first-col"; ?>">
			<div class="vc_column-inner ">
				<div class="wpb_wrapper">
					<div class="wpb_text_column wpb_content_element ">
						<div class="wpb_wrapper">
...
<?php foreach($row AS $k=>$v): ?>
	<div class="wpb_column vc_column_container vc_hidden-lg vc_hidden-md vc_col-sm-6 vc_col-xs-offset-1 vc_col-sm-offset-0 vc_col-xs-6 field-titles<?php echo " C_". str_replace($not_allowed, "_", strip_tags(strtolower($k)) );  if( $k=="DETAILS" ): echo " vc_hidden-xs vc_hidden-sm last-col"; endif; if( $k=="PLOT NO" ): echo " first-col"; endif; ?>">
		<div class="vc_column-inner ">
			<div class="wpb_wrapper">
				<div class="wpb_text_column wpb_content_element">
					<div class="wpb_wrapper">
	...
	<div class="wpb_column vc_column_container vc_col-md-1 vc_col-sm-6 vc_col-xs-4 field-values<?php echo " C_". str_replace($not_allowed, "_", strip_tags(strtolower($k)) ); if( $k=="DETAILS" ) echo " vc_hidden-xs vc_hidden-sm last-col"; if( $k=="PLOT NO" ) echo " first-col"; ?>">
		<div class="vc_column-inner ">
			<div class="wpb_wrapper">
				<div class="wpb_text_column wpb_content_element ">
					<div class="wpb_wrapper">
	...
	<?php if( $k=="DETAILS" ): ?>
	<div class="wpb_column vc_column_container vc_col-sm-12 vc_col-xs-12 field-value vc_hidden-lg vc_hidden-md">
		<div class="vc_column-inner ">
			<div class="wpb_wrapper">
				<div class="wpb_text_column wpb_content_element ">
					<div class="wpb_wrapper">
						<p class="view-plot"><?php echo $v; ?></p>
					</div>
				</div>
			</div>
		</div>
...
function property_links_func( $atts ) {
	...
	$filepath = get_attached_file( $spec_pdf['id'] );
	$spec_pdf['size'] = size_format(filesize($filepath), 2);
	
	$floorp_pdf = get_field('floorp_pdf');
	$filepath = get_attached_file( $floorp_pdf['id'] );
	if( $floorp_pdf ) $floorp_pdf['size'] = size_format(filesize($filepath), 2);
	
	$floorpb_pdf = get_field('floorpb_pdf');
	$filepath = get_attached_file( $floorpb_pdf['id'] );
	if( $floorpb_pdf ) $floorpb_pdf['size'] = size_format(filesize($filepath), 2);
	
	/*$data = array( 
			"spec_pdf" => $spec_pdf,
			"floorp_pdf" => $floorp_pdf,
			"floorpb_pdf" => $floorpb_pdf
		);
	...
	$html = '<a target="_blank" href="'.$spec_pdf['url'].'"><i class="vc_btn3-icon fa fa-chevron-down"></i> Download '.$spec_pdf['title'].' ('.$spec_pdf['size'].')</a>';
	if( $floorp_pdf ) $html.= '<br/><a target="_blank" href="'.$floorp_pdf['url'].'"><i class="vc_btn3-icon fa fa-chevron-down"></i> Download '.do_shortcode('[property-cat]').' floorplan ('.$floorp_pdf['size'].')</a>';
	if( $floorpb_pdf ) $html.= '<br/><a target="_blank" href="'.$floorpb_pdf['url'].'"><i class="vc_btn3-icon fa fa-chevron-down"></i> Download '.$floorpb_pdf['title'].' ('.$floorpb_pdf['size'].')</a>';
