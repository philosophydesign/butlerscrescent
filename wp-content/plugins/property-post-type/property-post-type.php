	...
	if ($post->post_type == 'property') {
		switch($hook){
			...
			case 'edit.php':
				...
				wp_enqueue_script( 'ajax-script', plugins_url( '/js/admin-ajax.js', __FILE__ ), array('jquery'), '1.0.0', true );
			break;
		}
		return;
	} else return;
...
function property_front_enqueue() {
	...
	*/
	...
	/* custom status appears for all types - need to only show on one */
	if(  in_array('property', array($_GET['post_type'], $_POST['post_type'])) || (strpos($_SERVER['REQUEST_URI'], 'properties') !== false) ) add_action( 'init', 'custom_post_status', 0 );
