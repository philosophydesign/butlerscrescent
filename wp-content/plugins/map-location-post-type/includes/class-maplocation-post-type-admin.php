...
class Maplocation_Post_Type_Admin {
	...
	public function init() {
		...
		// Add Email to column view
		//add_filter( 'manage_edit-' . $this->registration_handler->post_type . '_columns', array( $this, 'add_email_column'), 10, 1 );
		//add_action( 'manage_posts_custom_column', array( $this, 'display_email' ), 10, 1 );
		
		// Add address to column view
		add_filter( 'manage_edit-' . $this->registration_handler->post_type . '_columns', array( $this, 'add_address_column'), 10, 1 );
		add_action( 'manage_posts_custom_column', array( $this, 'display_address' ), 10, 1 );
		
		// Add geo code to column view
		add_filter( 'manage_edit-' . $this->registration_handler->post_type . '_columns', array( $this, 'add_geocode_column'), 10, 1 );
		add_action( 'manage_posts_custom_column', array( $this, 'display_geocode' ), 10, 1 );
	...
	/**
	 * Add column2 to post type list screen.
	 *
	 * @link http://wptheming.com/2010/07/column-edit-pages/
	 *
	 * @param array $columns Existing columns.
	 ...
	 * @return array Amended columns.
	 */
	public function add_email_column( $columns ) {
		
		//remove title
		//unset($columns['title']);
		//$columns['title'] =  'Email';

		$column_email = array( 'email' => __( 'Email', 'maplocation-post-type' ) );
		return array_slice( $columns, 0, 2, true ) + $column_email + array_slice( $columns, 1, null, true );
	}
	
	
	/**
	 * Custom column2 callback
	 *
	 * @global stdClass $post Post object.
	 *
	 * @param string $column Column ID.
	 */
	public function display_email( $column ) {
		switch ( $column ) {
			case 'email':
				//echo "Email Text";
				echo get_metadata('post', get_the_ID(), 'email-address', true);
				break;
		}
	}
	
	/**
	 * Add column3 to post type list screen.
	 *
	 * @link http://wptheming.com/2010/07/column-edit-pages/
	 *
	 * @param array $columns Existing columns.
	...
	public function add_address_column( $columns ) {
		$column_address = array( 'address' => __( 'Address', 'maplocation-post-type' ) );
		return array_slice( $columns, 0, 4, true ) + $column_address + array_slice( $columns, 1, null, true );
	}
	 ...
	 * @return array Amended columns.
	 */
	public function add_geocode_column( $columns ) {
		$column_geocode = array( 'geocode' => __( 'Geocode', 'maplocation-post-type' ) );
		return array_slice( $columns, 0, 5, true ) + $column_geocode + array_slice( $columns, 1, null, true );
	}
	
	/**
	 * Custom column3 callback
	 *
	 * @global stdClass $post Post object.
	 *
	 * @param string $column Column ID.
	 */
	public function display_address( $column ) {
		switch ( $column ) {
			case 'address':
				echo get_metadata('post', get_the_ID(), 'address', true);
				break;
		}
	}
	 ...
	 * @param string $column Column ID.
	 */
	public function display_geocode( $column ) {
		switch ( $column ) {
			case 'geocode':
				echo get_metadata('post', get_the_ID(), 'geo_code', true);
				break;
		}
	}
